...
export default function KadinPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/products?category=kadin')
      const data = await response.json()
      
      if (data.success) {
        // Ürünleri kadın sayfası için formatla
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-pink-600 to-purple-600 text-white py-16">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl font-bold mb-4">Kadın Kategorisi</h1>
          <p className="text-xl opacity-90">
            Kadınlar için şık ve modern ürünler
          </p>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['Pembe', 'Mavi', 'Kırmızı', 'Siyah', 'Beyaz'].map((color) => (
                        <button
                          key={color}
