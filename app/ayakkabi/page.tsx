...
export default function AyakkabiPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/products?category=ayakkabi')
      const data = await response.json()
      
      if (data.success) {
        // Ürünleri ayakkabı sayfası için formatla
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-orange-600 to-red-600 text-white py-16">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl font-bold mb-4">Ayakkabı Kategorisi</h1>
          <p className="text-xl opacity-90">
            Konforlu ve şık ayakkabılar
          </p>
        </div>
      </div>
                ...
                <div>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex space-x-2">
                      {['Siyah', 'Beyaz', 'Kahverengi', 'Gri', 'Mavi'].map((color) => (
                        <button
                          key={color}
