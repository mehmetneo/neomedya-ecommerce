...
export default function ErkekPage() {
  ...
  const [products, setProducts] = useState(erkekUrunleri)
  ...
  useEffect(() => {
    ...
    const handleProductsUpdated = (event: CustomEvent) => {
      ...
      const erkekProducts = updatedProducts.filter((product: any) => product.category === 'Erkek')
      setProducts(erkekProducts.length > 0 ? erkekProducts : erkekUrunleri)
    }
    ...
    if (savedProducts) {
      ...
      const erkekProducts = allProducts.filter((product: any) => product.category === 'Erkek')
      if (erkekProducts.length > 0) {
        setProducts(erkekProducts)
      }
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto">
          {/* Başlık */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Erkek Koleksiyonu</h1>
            <p className="text-gray-600">En trend erkek kıyafetleri ve aksesuarları</p>
          </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      {product.isSale ? (
                        <div>
                          <span className="text-lg font-bold text-gray-900">{product.price.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}</span>
                          <span className="text-sm text-gray-500 line-through ml-2">{product.originalPrice?.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}</span>
                        </div>
                      ) : (
                        <span className="text-lg font-bold text-gray-900">{product.price.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}</span>
                      )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <h3 className="text-lg font-semibold text-gray-900">Beden Seçin</h3>
                    <div className="flex space-x-2">
                  ...
                  {/* Renk Seçimi */}
                  <div className="space-y-3">
                    <h3 className="text-lg font-semibold text-gray-900">Renk Seçin</h3>
                    <div className="flex space-x-2">
                      {['Beyaz', 'Siyah', 'Mavi', 'Kırmızı', 'Yeşil'].map((color) => (
                        <button
                          key={color}
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      Sepete Ekle - {selectedProduct.price.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}
                    </button>
                    <button
                      onClick={closeProductModal}
                      className="w-full bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 px-6 rounded-lg text-lg font-medium transition-colors"
                    >
                      Kapat
                    </button>
                  </div>
